...
function agent_count(agent::Animal)
    return 1
end

function agent_count(agent::Plant)
    return agent.size / agent.max_size
end

function agent_count(agents::Vector{<:Agent})
    return sum(agent_count.(agents))
end

function agent_count(world::World)
    result = Dict{Symbol, Float64}()
    for (id, agent) in world.agents
        name = nameof(typeof(agent))
        if name in keys(result)
            result[name] += agent_count(agent)
        else
            result[name] = agent_count(agent)
        end
    end
...
println(agent_count(sheep))
println(agent_count([grass1, grass2, sheep, wolf]))
